...
29. CSV import: drag-and-drop upload supporting three formats (Movebank, Base Lunar, Ornitela), auto-detect format by separator/columns, preview with column mapping, batch insert with duplicate detection, auto-create individuals with metadata (taxon, sex from Base Lunar; device_id from Ornitela)
33. Ornitela CSV import: supports all 6 download formats (GPS, SENSORS, GPS+SENSORS, GPS_V2, SENSORS_V2, GPS+SENSORS_V2); flexible column detection with V1/V2 auto-mapping; GPS-only rows → GPS records; SENSORS-only rows → accelerometer records; mixed rows → both; speed converted km/h→m/s; device_id used as individual identifier; rows with lat/lon=0 skipped; V2 columns auto-mapped (tagID→device_id, DateTime_UTC→UTC_datetime, lat/lon, acceleration_x/y/z, etc.); unknown columns silently ignored; sub-format reported in response
